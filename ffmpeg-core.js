var createFFmpegCore = ( () => {
  ...
  return (async function(createFFmpegCore={}) {
    ...
    function saveToIndexedDB(db, storeName, key, value) {
      return new Promise((resolve, reject) => {
  ...
  function getFromIndexedDB(db, storeName, key) {
    return new Promise((resolve, reject) => {
      ...
      async function fetchAndStoreWASM() {
          const db = await openDB("FFmpegDB", "WASMStore");
      
          // Check if the WASM file is already in IndexedDB
          const cachedWASM = await getFromIndexedDB(db, "WASMStore", "ffmpeg-core");
          if (cachedWASM) {
              console.log("Using WASM from IndexedDB");
              return cachedWASM
          }
      
          // Fetch and store WASM file
          console.log("Fetching WASM from network");
          const response = await fetch("https://cdn.glitch.me/aa1ca3e9-405e-4783-9d34-df1ad34560ef/ffmpeg-core.wasm");
          const wasmArrayBuffer = await response.arrayBuffer();
          await saveToIndexedDB(db, "WASMStore", "ffmpeg-core", wasmArrayBuffer);
      ...
      function getBinaryPromise(binaryFile) {
          if (!wasmBinary && (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER)) {
              if (typeof fetch == "function" && !isFileURI(binaryFile)) {
                  return fetchAndStoreWASM().then(response => {
      ...
      function instantiateAsync(binary, binaryFile, imports, callback) {
          if (!binary && typeof WebAssembly.instantiateStreaming == "function" && !isDataURI(binaryFile) && !isFileURI(binaryFile) && !ENVIRONMENT_IS_NODE && typeof fetch == "function") {
              return fetchAndStoreWASM().then(arraybuff => {
                const wasmBlob = new Blob([arraybuff], { type: 'application/wasm' });
      ...
      var _ff_h264_cabac_tables = Module["_ff_h264_cabac_tables"] = 1534380;
      var ___start_em_js = Module["___start_em_js"] = 6052093;
      var ___stop_em_js = Module["___stop_em_js"] = 6052270;
